...
//double Length(Vector4 p1, Vector4 p2)
//{
//	LONG x1, y1, x2, y2;
//	USHORT depth1, depth2;
//
//	NuiTransformSkeletonToDepthImage( p1,  &x1,  &y1,  &depth1);
//	NuiTransformSkeletonToDepthImage( p2,  &x2,  &y2,  &depth2);
//
//	return sqrt(
//		pow(x1 - x2, 2) +
//		pow(y1 - y2, 2) +
...
{      
		...
		NuiTransformSkeletonToDepthImage(skel.SkeletonPositions[1], &x, &y, &depth);
		...
		/*
		SetDlgItemInt(m_hWnd, IDC_X_t, (UINT32)x, TRUE);
		SetDlgItemInt(m_hWnd, IDC_Y_coord, (UINT32)y, TRUE);
		SetDlgItemInt(m_hWnd, IDC_Z_coord, (UINT32)depth_temp, TRUE);
		*/
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
		...
		m_Points[i] = SkeletonToScreen(still, windowWidth, windowHeight);
    }
